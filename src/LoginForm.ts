...
export class LoginForm {
  ...
  handleEvent(e: Event) {
    if (e.currentTarget === this.showPassword) {
      ...
      const authenticationKeyPromise = <Promise<ArrayBuffer>>(
        passwordKeyPromise.then((key) => {
      ...
      const fetchDataPromise = authenticationKeyPromise
        .then((buf) => {
          const sPass = this.serverAccess.passwordStr;
          const sNamespace = this.serverAccess.namespaceStr;
          const user = this.username.value;
        ...
        .then(function (json: { data: string }) {
          return json.data;
        });
      Promise.all([encryptionKeyPromise, fetchDataPromise])
        .then((x) => {
          const [encKey, data] = x;
          this.encKey = encKey;
          const cipherTextBuf = hexStringToUint8Array(data).buffer;
          const ver = new DataView(cipherTextBuf).getUint32(0, true);
          if (ver !== 0) {
            return Promise.reject({
              type: 'E_CLIENT_ENCRYPTION_UNSUPPORTED',
              message: `client does not support version ${ver}`,
            });
          }
